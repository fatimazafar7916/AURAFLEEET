...
export const Integrations = () => {
  ...
  // Positions for logos around the center (x, y as percentages)
  const logoPositions = [
    { name: 'HubSpot', logo: 'https://upload.wikimedia.org/wikipedia/commons/3/3f/HubSpot_Logo.svg', x: 42, y: 15, color: '#FF7A59' },
    { name: 'Instagram', logo: 'https://upload.wikimedia.org/wikipedia/commons/a/a5/Instagram_icon.png', x: 52, y: 25, color: '#E1306C' },
    { name: 'Pipedrive', text: 'pipedrive', x: 68, y: 25, color: '#6B4FBB' },
    { name: 'Gmail', logo: 'https://upload.wikimedia.org/wikipedia/commons/7/7e/Gmail_icon_%282020%29.svg', x: 28, y: 35, color: '#EA4335' },
    { name: 'HQ', text: 'HQ', x: 22, y: 45, color: '#2A6FFF' },
    { name: 'WhatsApp', logo: 'https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg', x: 62, y: 42, color: '#25D366' },
    { name: 'TikTok', logo: 'https://upload.wikimedia.org/wikipedia/en/a/a9/TikTok_logo.svg', x: 30, y: 58, color: '#000000' },
    { name: 'Phone', text: '📞', x: 60, y: 62, color: '#10B981' },
    { name: 'Stripe', text: 'stripe', x: 68, y: 65, color: '#635BFF' },
    { name: 'RentSyst', text: 'RentSyst', x: 25, y: 72, color: '#4B5563' },
    { name: 'SMS', text: '💬', x: 42, y: 75, color: '#10B981' },
    { name: 'Twilio', logo: 'https://upload.wikimedia.org/wikipedia/commons/7/7e/Twilio-logo-red.svg', x: 52, y: 82, color: '#F22F46' },
  ];
    ...
    <section id="integrations" className="py-16 md:py-20 bg-white overflow-hidden">
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="flex justify-center">
          <div className="relative" style={{ width: '100%', maxWidth: 700, height: 500 }}>
            {/* Center Logo - Aiaura */}
              ...
              <div className="relative">
                <div className="w-24 h-24 rounded-3xl flex items-center justify-center"
                  style={{ 
                    ...
                    boxShadow: '0 0 40px rgba(16,185,129,0.3), 0 8px 32px rgba(0,0,0,0.1)' 
                  }}>
            ...
            {logoPositions.map((item, idx) => (
              <div
                key={idx}
                className="absolute flex flex-col items-center gap-2 cursor-pointer group transition-transform hover:scale-110"
              ...
              >
                ...
                {/* Logo container */}
                <div 
                  className="relative z-10 w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center transition-all bg-white"
                  style={{ 
                    boxShadow: `0 4px 20px ${item.color}25, 0 2px 8px rgba(0,0,0,0.08)`, 
                  ...
                  {item.logo ? (
                    <img 
                      ...
                      onError={(e: any) => { 
                        ...
                        e.target.parentNode.innerHTML=`<span style="font-size:14px;font-weight:700;color:${item.color}">${item.name.slice(0,2)}</span>`; 
                      }} 
                    />
                  ) : (
                    <span style={{ fontSize: item.text && item.text.length > 3 ? 11 : 20, fontWeight: 700, color: item.color }}>
                      {item.text}
                    </span>
                  )}
                </div>
                ...
                {hoveredLogo === item.name && (
                  <div 
                    className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap"
                    style={{ 
        ...
        {/* Bottom info */}
        <div className="mt-12 flex items-center justify-center px-6 py-4 rounded-2xl max-w-2xl mx-auto" style={{ background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
